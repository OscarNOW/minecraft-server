...
const blocks = JSON5.parse(fs.readFileSync(path.resolve(__dirname, '../../data/static/blocks.json')).toString())
...
function getStateId(blockName, state = {}) {
    ...
    block.states.forEach(({ name, values }) => {
        if (state[name] === undefined) throw new Error(`Unknown state for "${name}", expected one of ${require('util').inspect(values, { color: true })}, but got "${state[name]}" (${typeof state[name]})`)
