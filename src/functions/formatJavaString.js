        ...
        for (const { code, validInputTypes, validInputs } of replaceableStrings) {
            ...
            while (true) {
                ...
                if (replaceableStringIndex > args.length - 1)
                    throw new Error(`Not enough arguments for %${code} in "${string}". Got ${args.length}, expected at least ${replaceableStringIndex + 1}`); //todo: use CustomError

                const arg = args[replaceableStringIndex];

                if ((!validInputTypes.includes(typeof arg)) && (validInputs && !validInputs.includes(arg)))
                    throw new Error(`Invalid argument type for %${code} in ${string}`); //todo: use CustomError and make message clearer
